...
## 🧠 这个游戏是怎么用 vibe coding 做出来的

这是一次典型的 vibe coding 实践，整个过程就是"**描述 → 生成 → 试玩 → 反馈 → 迭代**"：

1. **描述需求**：向 AI 描述"我要做一个别踩白块小游戏，黑块下落，点黑得分，点白结束"
2. **迭代反馈**：玩一局 → 告诉 AI "太快了 / 方块会重叠 / 想加点音效" → AI 改
3. **逐步加料**：速度曲线、最高分、连击、粒子特效、Web Audio 音效、移动端适配……
4. **发布**：推到 GitHub + 开启 GitHub Pages，零构建直接上线

> 💡 vibe coding 的核心不是"让 AI 全自动"，而是**人负责判断和方向，AI 负责速度和细节**。
> 这个游戏从 0 到可玩只花了很短的时间，全靠"小步快跑 + 及时反馈"。
